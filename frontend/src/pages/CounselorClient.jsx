...
const INIT_CLIENTS = [
  {
    id: 1, name: '이은지', birth: '1995.06.12', gender: '여', phone: '010-1234-5678',
    status: '진행 중', keywords: ['거절연습', '자아경계', '대인관계'],
  ...
  {
    id: 2, name: '최민수', birth: '1988.11.20', gender: '남', phone: '010-9876-5432',
    status: '대기 중', keywords: ['번아웃', '직장스트레스', '수면장애'],
    survey: { reason: '직장/학업 스트레스', prev: '없음', goal: '무기력 해소와 일상 회복이 목표입니다.' },
    logs: [],
  },
  {
    id: 3, name: '박지연', birth: '2001.03.05', gender: '여', phone: '010-5555-4444',
    status: '진행 중', keywords: ['학업스트레스', '시험불안', '집중력'],
    survey: { reason: '직장/학업 스트레스', prev: '있음', goal: '시험 불안을 줄이고 집중력을 높이고 싶습니다.' },
    logs: [
      { id: 301, date: '2026.05.08', title: '1회차 상담 일지', content: '시험 불안과 완벽주의 성향 탐색. 인지왜곡 패턴 확인.' },
    ],
  },
  {
    id: 4, name: '김지아', birth: '1992.08.14', gender: '여', phone: '010-1111-2222',
    status: '종료', keywords: ['자존감', '대인관계', '자기이해'],
    survey: { reason: '자기이해 및 성장', prev: '없음', goal: '자존감 회복과 나다운 삶을 찾고 싶습니다.' },
    logs: [
      { id: 401, date: '2026.03.01', title: '1회차 상담 일지', content: '자존감 관련 핵심 신념 탐색. 어린 시절 경험과의 연결.' },
      { id: 402, date: '2026.04.20', title: '최종(10회차) 상담 일지', content: '10회기 프로그램 마무리. 변화 점검 및 종결 작업 진행.' },
    ],
  },
  {
    id: 5, name: '이하늘', birth: '1998.02.25', gender: '남', phone: '010-3333-7777',
    status: '대기 중', keywords: ['공황장애', '불안', '일상복귀'],
    survey: { reason: '우울/불안', prev: '있음', goal: '공황 증상을 조절하고 일상으로 복귀하고 싶습니다.' },
    logs: [],
  },
  {
    id: 6, name: '정민우', birth: '1985.12.01', gender: '남', phone: '010-8888-9999',
    status: '진행 중', keywords: ['가족갈등', '부부관계', '대화법'],
    survey: { reason: '갈등', prev: '없음', goal: '배우자와의 소통 방식을 개선하고 관계 회복을 원합니다.' },
    logs: [
      { id: 601, date: '2026.05.05', title: '1회차 상담 일지', content: '부부 갈등 패턴 파악. 비난-방어 사이클 교육 및 공감 대화법 소개.' },
      { id: 602, date: '2026.05.12', title: '2회차 상담 일지', content: '감정 표현 연습. 배우자 입장 취해보기 역할극 진행.' },
    ],
  },
];
...
const CounselorClient = () => {
  ...
  const autoKeywords = useMemo(() => {
    if (!client) return [];
    const text = [
      client.survey.reason, client.survey.goal,
      ...client.logs.map(l => l.content),
    ].join(' ');
          ...
          <ul className="cc-client-list">
            {filtered.map(c => (
              <li
                ...
                onClick={() => { setSelectedId(c.id); setOpenLogId(null); }}
              >
        ...
        <main className="cc-main">
          ...
          <div className="cc-profile-card">
            <div className="cc-profile-info">
              <span className={`cc-status-badge large ${STATUS_CLASS[client.status]}`}>{client.status}</span>
              <h2>{client.name} <small>{client.gender} · {client.birth} | {client.phone}</small></h2>
              {/* <p className="cc-phone"> </p> */}
              <button className="cc-btn cc-btn--outline" onClick={() => setSurveyModal(true)}>📋 사전 설문지 확인</button>
            </div>
            <button className="cc-btn cc-btn--primary" onClick={() => setLogModal({ open: true, editId: null, content: '' })}>
              + 새 일지 작성
            </button>
          </div>
          ...
          <section className="cc-history">
            ...
            {client.logs.length === 0
              ? <div className="cc-empty"><p>아직 상담 기록이 없습니다.</p><span>새 일지 작성 버튼으로 첫 기록을 남겨보세요.</span></div>
              : client.logs.map(log => (
                <div key={log.id} className="cc-log-group">
                  ...
                  >
                    ...
                    <div className="cc-log-text">
                      ...
                      <h4>{log.title}</h4>
                      <p>{log.content.substring(0, 40)}{log.content.length > 40 && '…'}</p>
                    </div>
                  ...
                  {openLogId === log.id && (
                    <div className="cc-log-detail fade-in">
                      <div className="cc-log-detail__actions">
                        <button className="cc-btn-sm cc-btn-sm--edit" onClick={() => setLogModal({ open: true, editId: log.id, content: log.content })}>수정</button>
                        <button className="cc-btn-sm cc-btn-sm--delete" onClick={() => setDeleteModal({ open: true, logId: log.id })}>삭제</button>
                      </div>
        ...
        <aside className="cc-utility">
          <div className="cc-utility-box">
            ...
            <div className="cc-keywords">
              {autoKeywords.map((kw, i) => <span key={i} className="cc-keyword">#{kw}</span>)}
            </div>
          </div>
          <div className="cc-utility-box">
            <h4>Quick Memo</h4>
      ...
      {deleteModal.open && (
        <div className="cc-overlay">
          <div className="cc-modal delete-modal">
            ...
            <p>삭제된 기록은 복구할 수 없습니다.<br />신중하게 결정해 주세요.</p>
