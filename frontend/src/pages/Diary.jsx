...
const diaryData = [
  {
    ...
    stress: 15,
    title: '오랜만에 느껴보는 여유',
    content: '주말에 비가 와서 꼼짝 못 하다가, 오늘 드디어 날씨가 개었다. 점심시간에 잠깐 회사 근처 공원을 산책했는데, 햇살이 너무 따뜻하고 바람도 선선해서 기분이 한결 나아졌다.',
    aiFeedback: '따뜻한 햇살과 산책으로 에너지를 충전하셨군요! 작은 일상에서 긍정적인 변화를 찾아내는 먹보님의 모습이 멋집니다.',
    tags: ['#산책', '#햇살', '#여유']
  },
  {
    id: 2,
    ...
    aiFeedback: '무기력함이 밀려오는 날이었군요. 계획을 지키지 못했다고 자책하기보다는, 몸과 마음이 쉬어가는 시간이라고 생각해주세요.',
    tags: ['#무기력', '#휴식필요']
  },
  {
    id: 3,
    ...
    stress: 80,
    title: '발표를 앞둔 긴장감',
    content: '내일 있을 프로젝트 발표 때문에 하루 종일 마음이 조마조마하다. 준비는 다 했지만 자꾸만 실수할 것 같은 기분이 든다.',
    aiFeedback: '중요한 일을 앞두고 느끼는 불안은 당연한 에너지가 될 수 있어요. 지금까지 준비한 과정을 믿고 심호흡을 크게 한 번 해보세요.',
    tags: ['#불안', '#발표준비']
  },
  {
    id: 4,
    ...
    aiFeedback: '분노는 당신의 경계가 침범당했을 때 나타나는 정당한 신호입니다. 다만, 그 화가 당신을 삼키지 않도록 차가운 물을 마시며 열을 식혀보세요.',
    tags: ['#분노', '#감정조절']
  },
  {
    id: 5,
    ...
    stress: 10,
    title: '잔잔한 저녁 식사',
    content: '좋아하는 음악을 들으며 요리를 하고 천천히 식사를 했다. 큰 이벤트는 없었지만 마음이 참 평온한 하루였다.',
    aiFeedback: '잔잔한 일상 속에서 평온함을 유지하는 것은 매우 건강한 상태입니다. 오늘의 이 평온함이 내일의 에너지가 되길 바랍니다.',
    tags: ['#평온', '#홈쿠킹']
  }
];
...
const MoodIcon = ({ mood, className = "" }) => {
  ...
  switch (mood) {
    case 'happy':   return <Smile   className={`${moodClass} ${className}`} />;
    case 'anxious': return <Annoyed className={`${moodClass} ${className}`} />;
    case 'sad':     return <Frown   className={`${moodClass} ${className}`} />;
    case 'angry':   return <Angry   className={`${moodClass} ${className}`} />;
    case 'calm':
    default:        return <Meh     className={`mw-icon-calm ${className}`} />;
  }
};
...
const MoodBadge = ({ mood }) => {
  ...
  return (
    <span className={`mw-mood-badge mw-mood-badge-${mood || 'calm'}`}>
      {labels[mood] || labels.calm}
    </span>
  );
};

const StressBadge = ({ level }) => {
  const colorClass = level > 70 ? 'mw-stress-badge-high' : level > 40 ? 'mw-stress-badge-medium' : 'mw-stress-badge-low';
  return (
        ...
        <div className="mw-header-inner">
          <div className="mw-header-left">
            ...
            <nav className="mw-nav">
              <a href="#">전문가 찾기</a>
              <a href="#">예약 관리</a>
              <a href="#" className="mw-nav-active">AI 일기</a>
              <a href="#">힐링 라운지</a>
            </nav>
          </div>
          ...
          <div className="mw-header-right">
            ...
            <div className="mw-profile-wrapper">
              <div className="mw-avatar">
                <img
                  src="https://api.dicebear.com/7.x/notionists/svg?seed=Felix&backgroundColor=FAF7F2"
                  alt="Profile"
                />
              </div>
              <span className="mw-username">먹보 님</span>
            </div>
          </div>
        </div>
      </header>
          ...
          <div className="mw-calendar-card mw-no-scrollbar">
            <div className="mw-calendar-header">
              ...
              <div className="mw-calendar-legend">
                <div className="mw-legend-item"><div className="mw-legend-dot mw-legend-dot-happy"></div>행복</div>
                <div className="mw-legend-item"><div className="mw-legend-dot mw-legend-dot-anxious"></div>불안</div>
                <div className="mw-legend-item"><div className="mw-legend-dot mw-legend-dot-angry"></div>화남</div>
                <div className="mw-legend-item"><div className="mw-legend-dot mw-legend-dot-sad"></div>우울</div>
                <div className="mw-legend-item"><div className="mw-legend-dot mw-legend-dot-calm"></div>평온</div>
              </div>
            </div>
            ...
            <div className="mw-calendar-grid">
              {calendarDays.map((day, idx) => {
                const diaryEntry = day ? diaryData.find(d => d.year === year && d.month === month && d.day === day) : null;
                ...
                return (
                  ...
                  >
                    {day && (
                      <>
                        <span className={`mw-cal-day-num ${colIndex === 0 ? 'mw-cal-day-num-sun' : colIndex === 6 ? 'mw-cal-day-num-sat' : ''}`}>
                          {day}
                        </span>
                        {diaryEntry && (
      ...
      <footer className="mw-footer">
        <div className="mw-footer-inner">
          <div className="mw-footer-brand">
            <div className="mw-footer-logo">MINDWELL</div>
            <p className="mw-footer-desc">
              AI기술을 활용하여 당신의 마음 건강을 진단하고,<br />
              가장 적합한 심리전문가를 연결해드립니다.
            </p>
          </div>
          <div>
            <div className="mw-footer-col-title">서비스</div>
          ...
          <div>
            <div className="mw-footer-col-title">고객지원</div>
            <ul className="mw-footer-list">
              <li>공지사항</li>
              <li>1:1 문의</li>
            </ul>
          ...
          <div>
            <div className="mw-footer-col-title">법적 고지</div>
            <ul className="mw-footer-list">
              <li className="mw-footer-list-bold">개인정보처리방침</li>
              <li>이용약관</li>
            </ul>
          </div>
        </div>
        <div className="mw-footer-copy">
        ...
        <div className="mw-modal-overlay">
          ...
          <div className="mw-modal-panel">
            ...
            <div className="mw-modal-body mw-no-scrollbar">
              ...
              <div className="mw-modal-content-box">
                {selectedDiary.content}
              </div>

              <div className="mw-modal-tags">
                {selectedDiary.tags.map(tag => (
                  <span key={tag} className="mw-modal-tag">{tag}</span>
                ))}
              </div>
              ...
              <div className="mw-ai-feedback-box">
                ...
                <p className="mw-ai-feedback-text">
