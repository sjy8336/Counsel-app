...
import { signUp, checkIdDuplicate } from '../api/auth.js';
...
export default function SignupPage() {
    ...
    const goNext = () => {
        if (currentStep === 1) {
            if (!formData.role) {
                showToast('가입 유형을 선택해 주세요.', 'error');
                return;
            }
            if (isEmailValid === false || !formData.email) {
                showToast('올바른 이메일 주소를 입력해 주세요.', 'error');
                return;
            }
            if (idStatus !== 'available') {
                showToast('아이디 중복 확인을 진행해 주세요.', 'warn');
                return;
            }
            if (!isPasswordValid) {
                showToast('비밀번호를 올바르게 입력해 주세요.', 'error');
                return;
            }
            if (!isPasswordMatch) {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.phone.length < 13) {
            showToast('올바른 휴대폰 번호를 입력해 주세요.', 'error');
            return;
        }
    ...
    const renderCalendarDays = () => {
        ...
        for (let i = 0; i < firstDay; i++) days.push(<div key={`e-${i}`} className="su3-cal-day su3-cal-day--empty" />);
                ...
                <div className="su3-card">
                    ...
                    <div className="su3-step-header">
                        <div className="su3-step-meta">
                            <span className="su3-step-badge">{isSuccess ? '가입 완료' : `STEP ${currentStep}/2`}</span>
                    ...
                    {isSuccess && (
                        <div className="su3-success">
                            ...
                            <p className="su3-success-desc">
                                <strong className="su3-success-name">{successInfo.name}</strong>님, MINDWELL
                                마인드케어에 오신 것을 환영합니다.
                                <br />
                    ...
                    {!isSuccess && currentStep === 1 && (
                        <div className="su3-step-body">
                            <div className="su3-section-head">
                                ...
                                <p className="su3-section-desc">
                                    상담을 신청하는 내담자인지, 상담을 이끄는 전문 상담사인지 선택해 주세요.
                                </p>
                            </div>

                            {/* role cards */}
                            <div className="su3-field">
                                <label className="su3-label">
                                    가입 유형 <span className="su3-required">*</span>
                                </label>
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    이메일 주소 <span className="su3-required">*</span>
                                </label>
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    아이디 <span className="su3-required">*</span>
                                </label>
                                ...
                                {idStatus === 'available' && (
                                    <p className="su3-hint su3-hint--success">사용 가능한 아이디입니다.</p>
                                )}
                                {idStatus === 'taken' && (
                                    <p className="su3-hint su3-hint--error">이미 사용 중인 아이디입니다.</p>
                                )}
                            </div>
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    비밀번호 <span className="su3-required">*</span>
                                </label>
                                <div className="su3-input-wrap">
                                ...
                                <div className="su3-strength">
                                    <div className="su3-strength-meta">
                                        <span>비밀번호 안전도</span>
                                        <span className={`su3-strength-label su3-strength-label--${pwStrength.color}`}>
                                            {pwStrength.label}
                                        </span>
                                    </div>
                                ...
                                {isPasswordValid === false && formData.password.length > 0 && (
                                    <p className="su3-hint su3-hint--error">
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    비밀번호 재확인 <span className="su3-required">*</span>
                                </label>
                                <div className="su3-input-wrap">
                                ...
                                {isPasswordMatch !== null && formData.confirmPassword.length > 0 && (
                                    <p
                                        className={`su3-hint${isPasswordMatch ? ' su3-hint--success' : ' su3-hint--error'}`}
                                    >
                    ...
                    {!isSuccess && currentStep === 2 && (
                        <form onSubmit={handleSubmit} className="su3-step-body">
                            <div className="su3-section-head">
                                ...
                                <p className="su3-section-desc">
                                    상담 서비스 이용을 위해 실명 정보 등록과 서비스 이용약관 동의가 필요합니다.
                                </p>
                            </div>
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    이름 (실명) <span className="su3-required">*</span>
                                </label>
                                <div className="su3-input-wrap">
                            ...
                            <div className="su3-field">
                                <label className="su3-label">
                                    휴대폰 번호 <span className="su3-required">*</span>
                                </label>
                                <div className="su3-input-wrap">
                                ...
                                {formData.phone.length > 0 && (
                                    <p
                                        className={`su3-hint${formData.phone.length === 13 ? ' su3-hint--success' : ' su3-hint--warn'}`}
                                    >
                                    ...
                                    {isCalendarOpen && (
                                        <div className="su3-cal-popup">
                                            ...
                                            <div className="su3-cal-weekdays">
                                                {['일', '월', '화', '수', '목', '금', '토'].map((d) => (
                                                    <div key={d} className="su3-cal-weekday">
                                                        {d}
                                                    </div>
                                                ))}
                                            </div>
                                ...
                                <div className="su3-field">
                                    ...
                                    <div className="su3-gender-grid">
                                        {[
                                            ...
                                            <label
                                                key={val}
                                                className={`su3-gender-card${formData.gender === val ? ' su3-gender-card--active' : ''}`}
                                            >
                                                <input
                                                    type="radio"
                                                    name="gender"
                                                ...
                                                />
                                                <span
                                                    className={`su3-gender-label${formData.gender === val ? ' su3-gender-label--active' : ''}`}
                                                >
                                                    {label}
                                                </span>
                                            </label>
                            ...
                            <div className="su3-agree-box">
                                <div className="su3-agree-header">
                                    ...
                                    <span className="su3-agree-header-text">
                                        상담데이터 제공 및 본인확인 동의(필수)
                                    </span>
                                </div>
                                <div className="su3-agree-footer-row">
                                    <label className="su3-agree-check-row su3-agree-check-row--nomargin">
                                        <div
                                            className={`su3-agree-check-box${agree ? ' su3-agree-check-box--checked' : ''}`}
                                        >
                                            <input
                                                type="checkbox"
                                                checked={agree}
                                            ...
                                            <Check
                                                size={12}
                                                className={`su3-agree-check-icon${agree ? ' su3-agree-check-icon--visible' : ''}`}
                                            />
                                        </div>
                                    ...
                                    >
                                        <ChevronDown
                                            size={20}
                                            className={`su3-agree-arrow${showDetail ? ' su3-agree-arrow--open' : ''}`}
                                        />
                                    </button>
                                </div>
                                {showDetail && (
                    ...
                    {!isSuccess && (
                        <div className={`su3-nav${currentStep === 1 ? ' su3-nav--center' : ''}`}>
                            {currentStep !== 1 && (
                                <button type="button" onClick={goPrev} className="su3-nav-prev">
                                    <ChevronLeft size={16} />
                                    이전
                                </button>
                            )}
