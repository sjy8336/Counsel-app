...
const App = () => {
    ...
    const getOffCardClassName = (isOff, hasReservations) => {
        ...
        className += isOff ? ' reservepanel-offday-card-active' : ' reservepanel-offday-card-default';
    ...
    const getPickerButtonClassName = (isActive) =>
        isActive ? 'reservepanel-picker-trigger reservepanel-picker-trigger-open' : 'reservepanel-picker-trigger';

    const getPickerChevronClassName = (isActive) =>
        isActive ? 'reservepanel-picker-chevron reservepanel-picker-chevron-open' : 'reservepanel-picker-chevron';
    ...
    const renderDetailModal = () => {
                        ...
                        <div className="reservepanel-modal-content">
                            <div className="reservepanel-modal-header-row">
                                <div className="reservepanel-person-header">
                                    ...
                                    <div>
                                        <h3 className="reservepanel-modal-title">{selectedReservation.client}님</h3>
                                        <p className="reservepanel-modal-subtitle">{selectedReservation.type}</p>
                                    </div>
                                </div>
                                <button
                                    onClick={() => setSelectedReservation(null)}
                            ...
                            <div className="reservepanel-detail-list">
                                ...
                                <div className="reservepanel-detail-item">
                                    ...
                                    <div>
                                        ...
                                        <p className="reservepanel-detail-text">{selectedReservation.topic}</p>
                                    </div>
                                </div>

                                <div className="reservepanel-status-row">
                                    <div className="reservepanel-status-row-inner">
                                        <span className="reservepanel-status-row-label">진행 상태</span>
                            ...
                            <div className="reservepanel-modal-actions reservepanel-modal-actions-split">
                                <button className="reservepanel-primary-button reservepanel-primary-button-full">
                                    일지 작성
                                </button>
                            </div>
                        </div>
        ...
        if (selectedDateDetails) {
            ...
            const dayBlocks = blockedSlots.filter((blockedSlot) => blockedSlot.date === selectedDateDetails);
            const dayReservations = reservations.filter((reservation) => reservation.date === selectedDateDetails);
                    ...
                    >
                        <div className="reservepanel-modal-content reservepanel-modal-scroll-body reservepanel-no-scrollbar">
                            <div className="reservepanel-modal-topline">
                                <h3 className="reservepanel-modal-title">{selectedDateDetails} 관리</h3>
                                <button
                                    onClick={() => {
                                        setSelectedDateDetails(null);
                                        setShowBlockInput(false);
                                    ...
                                    {hasReservations ? (
                                        <div className="reservepanel-card-stack">
                                            {dayReservations.map((reservation) => (
                                                <div key={reservation.id} className="reservepanel-booked-card">
                                                    <span className="reservepanel-booked-name">
                                                        {reservation.client}님
                                                    </span>
                                                    <span className="reservepanel-booked-time">{reservation.time}</span>
                                                </div>
                                            ))}
                                        </div>
                                    ) : (
                                        <p className="reservepanel-empty-copy">예약된 일정이 없습니다.</p>
                                    )}
                                </div>
                                ...
                                <div className={getOffCardClassName(isOff, hasReservations)}>
                                    <div className="reservepanel-offday-row">
                                        <div className="reservepanel-offday-info">
                                            <CalendarOff className={getOffIconClassName(isOff)} size={20} />
                                            <div>
                                        ...
                                        <button
                                            ...
                                            onClick={() =>
                                                isOff
                                                    ? setOffDays(offDays.filter((day) => day !== selectedDateDetails))
                                    ...
                                    {dayBlocks.length > 0 && (
                                        <div className="reservepanel-card-stack">
                                            {dayBlocks.map((block, index) => (
                                                ...
                                                >
                                                    <span className="reservepanel-blocked-time">{block.time}</span>
                                                    <button
                                                        onClick={() =>
                                                            setBlockedSlots(
                                                                blockedSlots.filter(
                                        ...
                                        <div className="reservepanel-block-form reservepanel-block-form-animate">
                                            <div className="reservepanel-block-form-grid">
                                                <TimePicker
                                                    ...
                                                    setIsActive={(value) => setActivePicker(value ? 'start' : null)}
                                                />
                                                <TimePicker
                                                    label="종료 시간"
                                                    value={endTime}
                                                    ...
                                                    setIsActive={(value) => setActivePicker(value ? 'end' : null)}
                                                />
                                            </div>
                                            <div className="reservepanel-block-actions">
                                                <button onClick={handleAddBlock} className="reservepanel-block-add">
                                                    <Check size={16} />
                                                    시간 추가
                                                </button>
                                                <button
    ...
    const renderCalendar = () => {
        ...
        for (let i = 0; i < firstDay; i += 1) {
            calendarDays.push(<div key={`empty-${i}`} className="reservepanel-calendar-empty" />);
        }
        ...
        for (let d = 1; d <= days; d += 1) {
            ...
            const dayBlocks = blockedSlots.filter((blockedSlot) => blockedSlot.date === dateString);
            ...
            calendarDays.push(
                ...
                >
                    <div className="reservepanel-calendar-dayhead">
                        <span className={getDateNumberClassName(isToday, isPast, isOff)}>{d}</span>
                    ...
                    <div className="reservepanel-calendar-entrylist">
                        {dayReservations.slice(0, 2).map((reservation) => (
                            ...
                            >
                                <span className="reservepanel-reservation-time">{reservation.time.split(' ')[0]}</span>
                                {reservation.client}
                            </div>
                        ))}
                        ...
                        {dayBlocks.length > 0 && (
                            <div className="reservepanel-blocked-chip">
                                <Slash size={8} />
                                <span className="reservepanel-blocked-chip-time">{dayBlocks[0].time}</span>
                                불가
                            </div>
                        )}
                    </div>
                    ...
                    <div className="reservepanel-list-card-main">
                        ...
                        <div>
                            <div className="reservepanel-list-card-head">
                                <h3 className="reservepanel-list-card-title">{reservation.client}님</h3>
            ...
            <main className="reservepanel-main reservepanel-no-scrollbar">
                <div className="reservepanel-hero">
                    <div>
                        ...
                        <p className="reservepanel-hero-copy">본인의 휴무와 상담 일정을 통합 관리합니다.</p>
                    </div>
                    <div className="reservepanel-view-toggle">
                        <button onClick={() => setViewMode('calendar')} className={getViewToggleClassName('calendar')}>
                            <CalendarIcon size={22} />
                        </button>
                        <button onClick={() => setViewMode('list')} className={getViewToggleClassName('list')}>
                            <List size={22} />
                        </button>
                    </div>
                </div>
                            ...
                            <div className="reservepanel-toolbar">
                                <div className="reservepanel-toolbar-main">
                                    ...
                                    <div className="reservepanel-month-nav">
                                        <button onClick={prevMonth} className="reservepanel-month-nav-button">
                                            <ChevronLeft size={20} />
                                        </button>
                                        <button onClick={nextMonth} className="reservepanel-month-nav-button">
