...
import {
    Settings, Bell, LogOut, ChevronRight, CalendarHeart, MessageSquareHeart,
    History, Wallet, Calendar, MessagesSquare, ArrowLeft, User, ShieldCheck,
    UserX, Mail, Phone, Camera, CheckCircle2, Lock, KeyRound, AlertCircle,
    ToggleRight, ToggleLeft, CalendarDays, Heart, ClipboardList, Target,
    Hash, Lightbulb, Check, HeadphonesIcon, HelpCircle, MessageCircle,
    FileText, MessageSquare, CreditCard, Receipt,
} from 'lucide-react';
...
const PAYMENT_HISTORY = [
    { id: 1, date: '2026.05.20', time: '14:32', counselor: '이지은', title: '1:1 심리상담 예약금',  amount: '20,000', method: '카카오페이',         status: '결제완료' },
    { id: 2, date: '2026.05.02', time: '10:15', counselor: '김민수', title: '1:1 심리상담 잔금',    amount: '60,000', method: '신용카드 (현대 12**)', status: '결제완료' },
    { id: 3, date: '2026.04.15', time: '16:40', counselor: '박지영', title: '커플 상담 예약금',     amount: '30,000', method: '네이버페이',           status: '취소완료' },
];
...
const PROFILE_SUBMENU = [
    { key: 'personal',     label: '개인정보 수정 및 보안', icon: User,  title: '개인정보 수정 및 보안', desc: '회원님의 소중한 정보와 비밀번호를 안전하게 관리하세요.' },
    { key: 'notification', label: '알림 설정',            icon: Bell,  title: '알림 설정',            desc: '상담 일정 및 서비스 소식을 전해드릴게요.' },
    { key: 'quit',         label: '서비스 탈퇴',          icon: UserX, title: '서비스 탈퇴',          desc: '탈퇴 시 소중한 기록들이 모두 삭제됩니다.' },
];
...
const NotificationSettings = ({ notifSettings, toggleNotif }) => (
    <div className="ns-card">
        <header className="ns-header">
            <div className="ns-header-icon-box"><Bell size={20} /></div>
        ...
        <main className="ns-list">
            {NOTIF_SETTINGS_DATA.map(({ key, title, desc }) => (
                <div key={key} className="ns-toggle-item">
                    ...
                    <button onClick={() => toggleNotif(key)} className={`ns-toggle-btn ${notifSettings[key] ? 'is-on' : 'is-off'}`}>
                        {notifSettings[key]
        ...
        <footer className="ns-footer-box">
            ...
            <p className="ns-footer-text">
                기기 전체 알림이 꺼져있을 경우 앱 설정을 켜도 알림이 전송되지 않습니다.
                휴대폰의 <span className="ns-footer-highlight">설정 &gt; 알림</span> 메뉴에서 마인드웰의 알림 허용 상태를 확인해 주세요.
            </p>
        </footer>
    </div>
);
...
export default function MyPage() {
    ...
    const [userInfo,             setUserInfo]             = useState({ name: '', id: '', username: '', email: '', phone: '', birth: '', gender: '', profile_img_url: '' });
    const [pwFields,             setPwFields]             = useState({ current: '', new1: '', new2: '' });
    const [pwLoading,            setPwLoading]            = useState(false);
    const [notifSettings,        setNotifSettings]        = useState({ session: true, marketing: false, report: true, service: true });
    ...
    useEffect(() => {
        const token = localStorage.getItem('access_token');
        const user  = localStorage.getItem('user');
        if (!token || !user) { setHistoryList([]); setCompletedConsultations(0); return; }

        const userObj = JSON.parse(user);
        const fetchLogs = userObj.role === 'client'
            ? getClientLogs(userObj.id)
            : getCounselorClients().then(clients => clients.find(c => String(c.id) === String(userObj.id))?.logs || []);
        ...
        fetchLogs
            ...
            .catch(() => { setHistoryList([]); setCompletedConsultations(0); });
    }, [userInfo.id]);
    ...
    const handleMenuClick = (id) => { setActiveMenu(id); setActiveSubMenu(null); };
    ...
    const handleChangePassword = async () => {
        if (!pwFields.current || !pwFields.new1 || !pwFields.new2) { alert('모든 비밀번호 입력란을 채워주세요.'); return; }
        if (pwFields.new1.length < 8 || !/[A-Za-z]/.test(pwFields.new1) || !/[0-9]/.test(pwFields.new1)) { alert('새 비밀번호는 8자 이상, 영문+숫자를 포함해야 합니다.'); return; }
        if (pwFields.new1 !== pwFields.new2) { alert('새 비밀번호가 일치하지 않습니다.'); return; }
        setPwLoading(true);
        try {
            await changePassword({ user_id: userInfo.id, current_password: pwFields.current, new_password: pwFields.new1 });
    ...
    const handleDeleteAccount = async () => {
        if (!withdrawAgree) { alert('유의사항 동의가 필요합니다.'); return; }
        if (!userInfo.id)   { alert('로그인 정보가 없습니다.'); return; }
    ...
    const handleUnfavorite = async (id, e) => {
        ...
        const token = localStorage.getItem('access_token');
        if (!token) { alert('로그인 후 이용 가능한 기능입니다.'); return; }
        try {
    ...
    const avatarSrc = (name, url) =>
        url?.trim() ? url : `https://api.dicebear.com/7.x/notionists/svg?seed=${name || 'default'}`;

    // ── 렌더 함수 ──

    const renderMobileBack = () => (
        <button className="mobile-back-btn mobile-back-btn--compact" onClick={handleBackToDashboard} aria-label="뒤로가기">
                ...
                <div className="report-card">
                    <div className="report-header">
                        <div>
                            ...
                            <p className="report-meta">{selectedConsultation.counselor} • {selectedConsultation.date} {selectedConsultation.time}</p>
                        </div>
                        <div className="icon-wrapper-outer"><div className="icon-wrapper-inner"><ClipboardList size={24} /></div></div>
                    </div>
                    <div className="report-body">
                        {[
                            { label: '상담 요약',    cls: 'summary-box',      textCls: 'summary-text',  val: selectedConsultation.summary  },
                            { label: '전문가 소견',  cls: 'feedback-wrapper', textCls: 'feedback-text', val: selectedConsultation.feedback },
                        ].map(({ label, cls, textCls, val }) => (
                            <section key={label} className="report-section">
                                <h4 className="section-title"><div className="title-indicator" />{label}</h4>
                                <div className={cls}><p className={textCls} dangerouslySetInnerHTML={{ __html: nl2br(val) }} /></div>
                            </section>
                        ))}
                        <section className="action-step-card">
                            <h4 className="action-step-title"><Target size={20} />다음 상담까지의 실천 과제</h4>
                            <p className="action-step-content" dangerouslySetInnerHTML={{ __html: nl2br(`"${selectedConsultation.nextStep || ''}"`) }} />
                        </section>
                    </div>
                    <div className="report-footer">
                        <p className="privacy-notice">이 기록은 오직 {userInfo.name}님과 담당 상담사만 확인할 수 있습니다.</p>
                    </div>
                </div>
            </div>
        );
                        ...
                        {historyList.length === 0 ? (
                            ...
                            <div key={item.id} onClick={() => setSelectedConsultation(item)} className="history-item-card">
                                <div className="history-item-main">
                                    <div className="history-icon-box"><CalendarDays size={24} /></div>
                                    <div>
                ...
                <div className="ph-grid">
                    <div className="ph-col-main">
                        <section className="ph-highlight-card">
                            <div className="ph-highlight-inner">
                                <div className="ph-highlight-meta">
                                    <span className="ph-status-badge"><CheckCircle2 size={14} /> 예약 확정</span>
                                ...
                                <div className="ph-highlight-payment-box">
                                    <div className="ph-payment-info">
                                        <div className="ph-payment-icon-wrap"><CreditCard className="ph-payment-icon" size={24} /></div>
                                        <div>
                                            <p className="ph-payment-label">결제 금액 (예약금)</p>
                                            <p className="ph-payment-amount">20,000<span className="ph-payment-amount-unit">원</span></p>
                                        </div>
                                    </div>
                        ...
                        <section className="ph-history-section">
                            <h4 className="ph-history-heading"><History className="ph-history-heading-icon" size={20} />전체 결제 내역</h4>
                                            ...
                                            {PAYMENT_HISTORY.map(item => (
                                                <tr key={item.id} className="ph-table-row">
                                                    <td className="ph-table-td">
                                                        ...
                                                        <span className="ph-counselor-name">{item.counselor} 상담사</span>
                                                    ...
                                                    <td className="ph-table-td ph-td-status-wrap">
                                                        <span className={`ph-status-pill ${item.status === '결제완료' ? 'ph-status-pill--complete' : 'ph-status-pill--cancel'}`}>
                                                            {item.status}
                                                        </span>
                                                    </td>
                                                </tr>
                    ...
                    <div className={`ph-col-side ${isMobile ? 'ph-col-side--mobile-spaced' : ''}`}>
                        <div className="ph-panel-card">
                            <h4 className="ph-panel-heading"><AlertCircle size={16} className="ph-panel-heading-icon" />예약금 및 환불 규정</h4>
                            <ul className="ph-refund-list">
                                {[
                                    { color: 'green', text: <>상담 예약 확정을 위해 <strong>20,000원의 예약금</strong> 결제가 필요합니다.</> },
                                    { color: 'green', text: <>상담 시작 <strong>24시간 전</strong> 취소 시 예약금은 100% 환불됩니다.</> },
                                    { color: 'amber', text: '상담 시작 24시간 이내 취소 또는 노쇼(No-Show) 시 예약금은 환불되지 않습니다.' },
                                ].map(({ color, text }, i) => (
                        ...
                        <div className="ph-panel-card">
                            <h4 className="ph-panel-heading"><Receipt size={16} className="ph-panel-heading-icon" />증빙 서류 발급 안내</h4>
                            <p className="ph-receipt-desc">결제하신 예약금 및 상담비에 대한 현금영수증 및 카드 전표가 필요한 경우 고객센터 혹은 1:1 문의를 통해 발급을 요청하실 수 있습니다.</p>
            ...
            <div className="fade-in mp-w-full">
                        ...
                        <div className="profile-image-container">
                            <input
                                ...
                                onChange={e => {
                                    ...
                                    reader.onload = ev => setUserInfo(prev => ({ ...prev, profile_img_url: ev.target.result, _profileImgFile: file }));
                                    reader.readAsDataURL(file);
                                }}
                            />
                            <div className="profile-image-wrapper">
                                <div className="profile-avatar" style={{ cursor: 'pointer' }} onClick={() => window.__profileImgInputRef?.click()}>
                                    <img src={profileImg} alt="User Profile" onError={e => { e.target.onerror = null; e.target.src = avatarSrc(userInfo.name); }} />
                                </div>
                                <button className="profile-camera-btn" onClick={e => { e.preventDefault(); window.__profileImgInputRef?.click(); }}>
                                    <Camera size={24} />
                                </button>
                            </div>
                            ...
                            <div className="profile-info-row">
                                <div className="profile-info-half">
                                    ...
                                    <div className="relative-input-box">
                                        ...
                                        <input type="text" className="custom-input" value={userInfo.name} onChange={e => setUserInfo(p => ({ ...p, name: e.target.value }))} />
                                    </div>
                                </div>
                                <div className="profile-info-half">
                                    <label className="input-label">성별</label>
                                    <div className="relative-input-box">
                                        <input type="text" className="custom-input bg-readonly" value={userInfo.gender === 'female' ? '여성' : userInfo.gender === 'male' ? '남성' : ''} readOnly />
                                    </div>
                                </div>
                            </div>
                            <div className="profile-info-row">
                                <div className="profile-info-half">
                                    ...
                                    <div className="relative-input-box">
                                        ...
                                        <input type="date" className="custom-input" value={userInfo.birth} onChange={e => setUserInfo(p => ({ ...p, birth: e.target.value }))} />
                                    </div>
                                </div>
                                <div className="profile-info-half">
                                    ...
                                    <div className="relative-input-box">
                                        ...
                                        <input type="tel" className="custom-input" value={userInfo.phone} onChange={e => setUserInfo(p => ({ ...p, phone: e.target.value }))} />
                                    </div>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="profile-readonly-grid">
                        ...
                        <div className="grid-col-1">
                            ...
                            <div className="relative-input-box">
                                ...
                                <input type="email" className="custom-input" value={userInfo.email} onChange={e => setUserInfo(p => ({ ...p, email: e.target.value }))} />
                            </div>
                ...
                <div className="profile-edit-section">
                    <h4 className="security-section-title">
                        <div className="security-icon-box"><ShieldCheck size={24} /></div>
                        로그인 보안 설정
                    </h4>
                    <div className="security-fields-layout">
                        <div>
                            ...
                            <div className="relative-input-box">
                                ...
                                <input type="password" placeholder="현재 비밀번호를 입력해 주세요" className="custom-input" value={pwFields.current} onChange={e => setPwFields(f => ({ ...f, current: e.target.value }))} autoComplete="current-password" disabled={pwLoading} />
                            </div>
                        </div>
                            ...
                            {[
                                ...
                                <div key={field}>
                                    ...
                                    <div className="relative-input-box">
                                        ...
                                        <input type="password" placeholder={placeholder} className="custom-input" value={pwFields[field]} onChange={e => setPwFields(f => ({ ...f, [field]: e.target.value }))} autoComplete="new-password" disabled={pwLoading} />
                                    </div>
                                </div>
                            ))}
                        </div>
                        ...
                        <div className="profile-action-row">
                            <button className={`pw-change-btn ${pwLoading ? 'is-loading' : ''}`} onClick={handleChangePassword} disabled={pwLoading}>
                    ...
                    <div className="wd-top-grid">
                        ...
                        <div className="wd-stats-column">
                            {[
                                ...
                                { label: '완료한 상담', value: completedConsultations, unit: '회', warning: '기록 전체 삭제' },
                            ].map(({ label, value, unit, warning }) => (
                                <div key={label} className="wd-stat-card">
                                    <p className="wd-stat-label">{label}</p>
                                    <div><span className="wd-stat-value">{value}</span><span className="wd-stat-unit">{unit}</span></div>
                    ...
                    <div className="wd-bottom-card">
                        <label className="wd-agree-box">
                            <div className="wd-checkbox-wrapper">
                                <input type="checkbox" checked={withdrawAgree} onChange={e => setWithdrawAgree(e.target.checked)} />
                                <div className="wd-check-display">{withdrawAgree && <Check size={11} strokeWidth={4} />}</div>
                            </div>
                            <span className="wd-agree-text">위 유의사항을 모두 확인하였으며 이에 동의합니다.</span>
                        </label>
                        <button className="wd-delete-btn" onClick={handleDeleteAccount} disabled={!withdrawAgree || withdrawLoading}>
                        ...
                        <div className="wd-suggestion-box">
                            ...
                            <p>
                                탈퇴 대신{' '}
                                <span className="wd-highlight" onClick={() => setActiveSubMenu('notification')}>알림 설정</span>
            ...
            <div className={isMobile ? 'mp-mobile-top-offset-48' : ''}>
                ...
                {inquiryLoading ? (
                    ...
                    <div className="inq-empty-state">
                        <div className="inq-empty-icon"><MessageSquare size={32} /></div>
                    ...
                    <div className="inq-list">
                        {inquiryList.map(item => (
                            <div key={item.id} className="inq-card">
                                <div className="inq-card-header" onClick={() => setOpenInquiryId(openInquiryId === item.id ? null : item.id)}>
                                    <div className={`inq-status-dot ${item.status === 'pending' ? 'dot-pending' : 'dot-done'}`} />
                                    <div className="inq-meta">
                                        <div className="inq-top-row">
                                            <span className={`inq-badge ${item.status === 'pending' ? 'badge-pending' : 'badge-done'}`}>
                                                {item.status === 'pending' ? '처리 중' : '답변 완료'}
                                            </span>
                                            <span className="inq-date">{item.created_at ? new Date(item.created_at).toLocaleDateString() : ''}</span>
                                        </div>
                                        <p className="inq-title">{item.title}</p>
                                        {openInquiryId !== item.id && <p className="inq-preview">{item.content}</p>}
                                    </div>
                                    <ChevronRight size={18} className={`inq-arrow-icon ${openInquiryId === item.id ? 'inq-arrow-open' : ''}`} />
                                </div>
                                    ...
                                    <div className="inq-body">
                                        ...
                                        {item.answer ? (
                                            <div className="inq-answer-block">
                                                <div className="inq-answer-label"><CheckCircle2 size={13} />관리자 답변</div>
                ...
                <ul className="mypage-list mypage-list-grid">
                    {favoritesLoading ? (
                        ...
                        const specialties  = item.field
                            ? item.field.split(',').map(f => f.trim()).filter(Boolean)
                            : (Array.isArray(item.specialties) ? item.specialties.map(s => s.specialty_name || s.name || s).filter(Boolean) : []);
                        const shown = specialties.slice(0, 3);
                        const extra = specialties.length - 3;
                        const src   = item.profile_img_url || `https://api.dicebear.com/7.x/notionists/svg?seed=${name}`;

                        return (
                            <li key={itemId} className="mypage-list-item mypage-favorite-item" onClick={() => navigate('/counselor/' + itemId, { state: { isLiked: true } })}>
                                <div className="mypage-favorite-heart" onClick={e => handleUnfavorite(itemId, e)} title="찜 취소">
                                ...
                                <div className="mypage-favorite-content">
                                    ...
                                    <div className="mypage-list-meta">
                                        {item.category && <span className="mypage-list-category">{item.category}</span>}
                                        {shown.map((f, i) => <span key={i} className="mypage-list-field">{f}</span>)}
                                        {extra > 0 && <span className="mypage-list-field-extra">+{extra}</span>}
                                    </div>
                                    {item.intro && <div className="mypage-list-intro">{item.intro || item.description}</div>}
                                    {item.price && (
                                        <div className="mypage-list-price">
                                            <span className="price-value">{item.price || item.consultation_price}</span>
            ...
            <div className="fade-in mp-w-full">
                <div className="submenu-container">
                    ...
                    <div className="submenu-content-area">
                        ...
                        {activeSubMenu === 'notification' && <NotificationSettings notifSettings={notifSettings} toggleNotif={toggleNotif} />}
                    </div>
                </div>
            </div>
        );
                    ...
                    <div className="cmp-list-card">
                        {notifications.length === 0 ? (
                            <div className="cmp-notif-empty">
                                <div className="cmp-notif-empty-icon"><Bell size={22} /></div>
                                ...
                                {visible.map(item => (
                                    <div key={item.id} className={`cmp-notif-item${!item.read ? ' unread' : ''}`}>
                                        ...
                                        <div className="cmp-notif-meta">
                                            <span className="cmp-notif-time">{new Date(item.created_at).toLocaleString()}</span>
                                            {!item.read && <span className="cmp-notif-dot" />}
                                        </div>
                                    </div>
                                ))}
                                {hasMore && (
                                    <button className="cmp-notif-more-btn" onClick={() => setShowAllNotifications(v => !v)}>
    ...
    const renderContent = () => {
        ...
        if (activeMenu === 'profile') return (
            <div className="fade-in fade-in--relative">
                {isMobile && (
                    <button className="mobile-back-btn mobile-back-btn--flush" onClick={handleBackToDashboard} aria-label="뒤로가기">
                        <ChevronRight className="mp-rotate-180" size={22} />
                    </button>
                )}
                <div className={isMobile ? 'mp-mobile-top-offset-48' : ''}>
                    <h3 className={`account-main-title ${isMobile ? 'account-main-title--mobile' : 'account-main-title--desktop'}`}>계정 설정</h3>
                        ...
                        {PROFILE_SUBMENU.map(({ key, label, icon: Icon, key: type }) => (
                            <div key={key} className="setting-item-card" onClick={() => setActiveSubMenu(key)}>
                                <div className={`setting-item-left is-${key}`}>
                                    <div className={`setting-item-icon-box is-${key}`}><Icon size={20} /></div>
        ...
        return (
            <div className="fade-in">
                <div className="dash-status-grid">
                    {[
                        { icon: <Wallet size={18} />,      bg: 'bg-amber', label: '결제 및 이용내역', value: ticketCount,            unit: '회', onClick: () => handleMenuClick('tickets') },
                        { icon: <CalendarHeart size={18} />, bg: 'bg-blue',  label: '대기 중인 예약',  value: 1,                      unit: '건', onClick: () => navigate('/reserve') },
                        { icon: <History size={18} />,      bg: 'bg-rose',  label: '완료한 상담',     value: completedConsultations,  unit: '회', onClick: () => handleMenuClick('history') },
                        ...
                        <div key={label} className="dash-status-item" onClick={onClick}>
                            ...
                            <p className="status-value">{value}<span className="unit">{unit}</span></p>
                        </div>
                    ))}
                </div>
                ...
                <div className="dash-section-header">
                    <h3 className="section-title"><History size={20} className="mp-history-title-icon" /> 최근 상담 기록</h3>
                    <button onClick={() => handleMenuClick('history')} className="section-more-btn">전체보기</button>
                </div>
                <div className="dash-history-list">
                    {historyList.length > 0 ? historyList.slice(0, 2).map(item => (
                        ...
                        <div className="dash-history-card dash-history-card--empty">
                            <div className="history-info-group">
                                <div className="history-icon-box"><MessageSquareHeart size={20} /></div>
                                <div>
                                    <div className="history-title-row"><p className="counselor-name">상담 기록이 없습니다</p></div>
                ...
                <section className="mobile-only-menu mobile-only-menu--spaced">
                    ...
                    <div className="mobile-menu-group mp-mt-4">
                        <div onClick={() => handleMenuClick('profile')} className="mobile-menu-item border-b">
                            <span>계정 설정</span><ChevronRight size={16} />
                        </div>
        ...
        <div className="mypage-layout-root">
            ...
            <aside className="mypage-sidebar">
                ...
                <nav className="sidebar-nav-list">
                    <div onClick={() => handleMenuClick('dashboard')} className={`sidebar-nav-item ${activeMenu === 'dashboard' ? 'is-active' : ''}`}>
                        <Settings size={20} /><span>대시보드</span>
                    </div>
                    <div onClick={() => setActiveMenu('notifications')} className={`sidebar-nav-item ${activeMenu === 'notifications' ? 'is-active' : ''}`}>
                        <Bell size={20} /><span>알림센터</span>
                    </div>
                    {MENU_ITEMS.map(({ id, label, icon: Icon }) => (
                        <div key={id} onClick={() => handleMenuClick(id)} className={`sidebar-nav-item ${activeMenu === id ? 'is-active' : ''}`}>
                            <Icon size={20} /><span>{label}</span>
                        </div>
                    ))}
                </nav>
                <div className="sidebar-footer-nav">
                    <div className={`sidebar-nav-item${activeMenu === 'support' ? ' is-active' : ''} mp-cursor-pointer`} onClick={() => setActiveMenu('support')}>
                        <HeadphonesIcon size={20} /><span>고객센터</span>
                    </div>
                        ...
                        <div className="user-profile-summary" onClick={() => handleMenuClick('dashboard')}>
                            <div className="user-avatar-box">
                                <img
                                    ...
                                    alt="User"
                                    onError={e => { e.target.onerror = null; e.target.src = avatarSrc(userInfo.name); }}
                                />
                            </div>
